...
if(isset($_POST['service_id']) && isset($_POST['counter_block']) && isset($_POST['limit_comment']) && isset($_POST['offset_comment'])){
  ...
  $data = $conn->prepare("SELECT id_author FROM adverts WHERE id_advert = :service_id");
  $data->bindParam(":service_id", $service_id, PDO::PARAM_INT);
  $data->execute();
  ...
  WHERE adv.is_service = 1 AND adv.id_advert = :service_id");
  $data->bindParam(":service_id", $service_id, PDO::PARAM_INT);
  $data->execute();

  echo "<div id='second_main_section' class='main-section-div service_details'>";
  if($_SESSION['language'] == "ENG"){
    echo "<button type='button' class='btn btn-dark btn-lg back_button' onclick='services_on()'><i class='bi bi-arrow-left'></i> Back to services</button>";
  }else if($_SESSION['language'] == "PL"){
    echo "<button type='button' class='btn btn-dark btn-lg back_button' onclick='services_on()'><i class='bi bi-arrow-left'></i> Powrót do usług</button>";
  }
                  ...
                  foreach ($row as $field => $value){
                    ...
                    if($counter1 == 6){
                      if((isset($_SESSION["is_admin"]) && $_SESSION["is_admin"] == 1) || $author_id == $_SESSION["user_id"]){
                        if($_SESSION['language'] == "ENG"){
                          echo "<button type='button' class='btn btn-danger btn-lg details' onclick='delete_advert(" . $advert_id . ")'>Delete service</button>";
                        }else if($_SESSION['language'] == "PL"){
                          echo "<button type='button' class='btn btn-danger btn-lg details' onclick='delete_advert(" . $advert_id . ")'>Usuń usługę</button>";
                        }
                      }
                      echo "</div>";
  ...
  WHERE adv.is_service = 1 AND adv.id_author = :user_id ");
  ...
  while ($row = $data->fetch(PDO::FETCH_ASSOC))
  {
        ...
        echo "<div class='container'>";
          echo "<div class='main_box author_box'>";
            echo "<div class='col-lg-12 col-md-12 col-sm-12 text-center'>";
              ...
              echo "<div class='col-lg-12 col-md-12 col-sm-12 text-center'>";
                if($_SESSION['language'] == "ENG"){
                  echo "<button type='button' class='btn btn-primary btn-lg' onclick='add_comment_to_service(" . $advert_id . ", " . $_SESSION['user_id'] . ", " . $counter_block . ")'>Send comment</button>";
                }else if($_SESSION['language'] == "PL"){
                  echo "<button type='button' class='btn btn-primary btn-lg' onclick='add_comment_to_service(" . $advert_id . ", " . $_SESSION['user_id'] . ", " . $counter_block . ")'>Wyślij komentarz</button>";
                }
              echo "</div>";
    ...
    WHERE com.id_advert = :service_id ORDER BY com.comment_added_date DESC");
    $data_all->bindParam(":service_id", $service_id, PDO::PARAM_INT);
    $data_all->execute();
    ...
    WHERE com.id_advert = :service_id ORDER BY com.comment_added_date DESC LIMIT {$limit_comment} OFFSET {$offset_comment}");
    $data->bindParam(":service_id", $service_id, PDO::PARAM_INT);
    $data->execute();
                ...
                foreach ($row as $field => $value){
                  ...
                  if($counter1 == 4){
                    ...
                    if(isset($_SESSION["is_admin"]) && $_SESSION["is_admin"] == 1){
                      if($_SESSION['language'] == "ENG"){
                        echo "<button type='button' class='btn btn-danger btn-lg details' onclick='delete_comment_from_service(" . $service_id . ", " . $comment_id . ", " . $counter_block .  ")'>Delete comment</button>";
                      }else if($_SESSION['language'] == "PL"){
                        echo "<button type='button' class='btn btn-danger btn-lg details' onclick='delete_comment_from_service(" . $service_id . ", " . $comment_id . ", " . $counter_block .  ")'>Usuń komentarz</button>";
                      }
                    }
                  }
  ...
  $loaded_service_comments = $counter_block_comment  + $offset_comment;
        ...
        echo "<div class='container'>";
          echo "<div class='bottom_nav'>";
            echo "<div class='col-lg-4 col-md-4 col-sm-4 text-center'>";
              if($loaded_service_comments > 5){
                if($_SESSION['language'] == "ENG"){
                 echo "<button type='button' class='btn btn-dark btn-lg' onclick='previous_comments_service(" . $service_id . ", " . $counter_block  . ")'>Previous</button>";
               }else if($_SESSION['language'] == "PL"){
                 echo "<button type='button' class='btn btn-dark btn-lg' onclick='previous_comments_service(" . $service_id . ", " . $counter_block  . ")'>Poprzednia</button>";
               }
              }
            echo "</div>";
            echo "<div class='col-lg-4 col-md-4 col-sm-4'>";
              if($loaded_service_comments  > $results){
                $loaded_service_comments  = $results;
              }
              echo "<p class='bottom_nav_pages'>" . $loaded_service_comments  . "/" . $results . "</p>";
            echo "</div>";
            echo "<div class='col-lg-4 col-md-4 col-sm-4 text-center'>";
              if($loaded_service_comments  < $results){
                if($_SESSION['language'] == "ENG"){
                  echo "<button type='button' class='btn btn-dark btn-lg' onclick='next_comments_service(" . $service_id . ", " . $counter_block  . ")'>Next</button>";
                }else if($_SESSION['language'] == "PL"){
                  echo "<button type='button' class='btn btn-dark btn-lg' onclick='next_comments_service(" . $service_id . ", " . $counter_block  . ")'>Następna</button>";
                }
              }
            echo "</div>";
          echo "</div>";
