...
if(isset($_POST['offset']) && isset($_POST['limit'])){
  ...
  $data_all = $conn->prepare("SELECT adv.advert_added_date, us.username, adv.place_name, adv.description, adv.advert_name, img.photo_url FROM adverts AS adv
  INNER JOIN images AS img ON adv.id_advert = img.id_advert INNER JOIN users AS us ON adv.id_author = us.id_user
  WHERE adv.is_musical_instrument = 1 AND adv.id_author = :id_user AND adv.is_archived = 0");
  ...
  INNER JOIN images AS img ON adv.id_advert = img.id_advert INNER JOIN users AS us ON adv.id_author = us.id_user
  WHERE adv.is_musical_instrument = 1 AND adv.id_author = :id_user AND adv.is_archived = 0 ORDER BY adv.advert_added_date DESC LIMIT {$limit} OFFSET {$offset}");
          ...
          echo "<div class='main_box left_image'>";
                  ...
                  foreach ($row as $field => $value){
                    ...
                    if($counter1 == 6){
                      if((isset($_SESSION["user_id"]))){
                        if($_SESSION['language'] == "ENG"){
                          echo "<button type='button' class='btn btn-primary btn-lg details' onclick='one_advert_on(" . $advert_id . ", " . $counter_block . ")'>Check details</button>";
                        }else if($_SESSION['language'] == "PL"){
                          echo "<button type='button' class='btn btn-primary btn-lg details' onclick='one_advert_on(" . $advert_id . ", " . $counter_block . ")'>Sprawdź szczegóły</button>";
                        }
                      }
                      echo "</div>";
                      echo "<div class='col-lg-6 col-md-12 col-sm-12 col-pad'>";
